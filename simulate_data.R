...
make.data <- function(nsp=64, ## number of species
                      nsite=15, ## number of sites
                      nyr=5, ## number of years
                      nrep=5, ## number of samples within a year
                      ...
                      sigma.p.0=0.5, ## variation in intercepts for
                                     ## detectability
                      mu.psi.beta=0, ## mean response to environmental
                                     ## gradient 
                      sigma.psi.site=0.1, ## variation in site random
                                          ## effect for occupancy
                      sigma.psi.yr=0.1, ## variation in year random
                                          ## effect for occupancy
                      ...
                      mu.p.beta=0, ## mean detection response to environment
                      sigma.p.beta=0, ## variance of detection
  ...
  alpha.spp <-
    rnorm(nsp, mean=mu.psi.0, sd=sigma.psi.0) + psi.0.trait*spp.trait
  ...
  ## Generate random effect of year on occupance
  yr.effect <- rnorm(nyr, mean=0, sd=sigma.psi.yr)
  
  ## Combine the above components to generate array of expected
  ## occupancies
  ##
  psi.indices <- expand.grid(sp=1:nsp, site=1:nsite, yr=1:nyr)
  ##
  ...
  ran.yr   <- yr.effect[psi.indices$yr]
  ## occupancy probability for each species/site/year
  psi <- array(invlogit(alpha + beta*env + ran.site + ran.yr),
               dim=c(nsp,nsite,nyr)) 
  
  ## Plot image of mean occupancy probability across years if desired
  if(psi.image==TRUE) {
    quartz()
  ...
  ZZ <- array(rbinom(nsp*nsite*nyr, 1, psi), dim=c(nsp,nsite,nyr))

  ## Plot image of mean occupancy states across years if desired
  if(occ.image==TRUE) {
    quartz()
  ...
  ##
  det.beta <- rnorm(nsp, mean=mu.p.beta, sd=sigma.p.beta)
  ...
  det <- array(det.pre, dim=c(nsp,nsite,nyr,nrep))


  ## Generate observation matrix
  EE <- array(ZZ, dim=dim(det)) * det
  XX <- array(rbinom(nsp*nsite*nyr*nrep, 1, prob=EE), dim=dim(det))
  ## Add dimension names
  ...
  ## Plot image of mean detection history across years if desired
  if(det.image==TRUE) {
    quartz()
